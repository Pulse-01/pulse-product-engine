  ...
  async function previewPrices(event) {
    ...
    try {
      ...
      output.innerHTML = `
        ...
        <div class="cc-preview-filters" role="group" aria-label="Preview filters">${filters.map(([filter, label], index) => `<button type="button" class="cc-preview-filter${index === 0 ? ' is-active' : ''}" data-preview-filter="${filter}">${label}</button>`).join('')}</div>
